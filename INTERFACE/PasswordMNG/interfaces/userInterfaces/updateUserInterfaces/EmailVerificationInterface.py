...
class EmailVerificationInterface(UserControl):
    def __init__(self):
        super().__init__()
        self.email = ''
        self.new_username = ''
    ...
    def get_update_data(self):
        ...
        update_data = main.update_user_storage.get_update_data()

        self.email = update_data.get('email')
        self.new_username = update_data.get('new_username')
        self.new_email = update_data.get('new_email')
    ...
    def build(self):
        ...
        def send_code():
            ...
            user_response = user_request.email_verify(self.email, _code_textField.value, self.new_email, self.new_username, CaptureToken().capture())

            match user_response.status_code:
                case 200:
                    import main

                    main.user_data_storage.set_user_data(self.new_email, self.new_username, CaptureToken().capture())
                    main.update_user_storage.clear()
                    main.info_data_storage.set_info_data("Email changed successfully!")
